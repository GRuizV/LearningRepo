...
-- @block /*UDACITY EXERCISES*/


-- @block // Find the one food that is eaten by only one animal 
...
-- @block // List all the taxonomic orders.
       ...
       Be careful:
              Each of these tables has a column "name", but they don't have the same meaning!  
              
              animals.name is an animal's individual name.  
              taxonomy.name is a species' common name (like 'brown bear').  
              And ordernames.name is the common name of an order (like 'Carnivores').
       */

       SELECT ordernames.t_order, count(*) as num
